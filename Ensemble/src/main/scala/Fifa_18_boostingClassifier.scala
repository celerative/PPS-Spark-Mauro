import org.apache.spark.ml.classification.{BoostingClassificationModel, BoostingClassifier, DecisionTreeClassifier}
...
object Fifa_18_boostingClassifier {
  def main(args: Array[String]){
    ...
    val boostingClassifier = new BoostingClassifier()
      .setBaseLearner(baseClassifier)
      .setMaxIter(10)

    val model = boostingClassifier.fit(train)
    ...
    val paramGrid = new ParamGridBuilder()
      .addGrid(boostingClassifier.learningRate, Array(0.3,0.8,1.0))
      .build()

    val cv = new CrossValidator()
      .setEstimator(boostingClassifier)
      .setEvaluator(re.setLabelCol(boostingClassifier.getLabelCol)
        .setPredictionCol(boostingClassifier.getPredictionCol))
    ...
    println(cvModel.bestModel.asInstanceOf[BoostingClassificationModel].getLearningRate)

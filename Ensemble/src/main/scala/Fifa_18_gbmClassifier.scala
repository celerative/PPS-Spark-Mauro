import org.apache.spark.ml.classification.{DecisionTreeClassifier, GBMClassificationModel, GBMClassifier, RandomForestClassifier, StackingClassifier}
...
object Fifa_18_gbmClassifier {
  def main(args: Array[String]){
    ...
    val baseClassifier = new DecisionTreeRegressor()

    val gbmClassifier = new GBMClassifier().setBaseLearner(baseClassifier).setMaxIter(10)

    val model = gbmClassifier.fit(train)

    val predicted = model.transform(test)
    predicted.show()

    val re = new MulticlassClassificationEvaluator().setMetricName("accuracy")

    println(re.evaluate(predicted))

    val paramGrid = new ParamGridBuilder()
      .addGrid(gbmClassifier.learningRate, Array(0.1, 1.0))
      .addGrid(gbmClassifier.maxIter, Array(1, 10))
      .build()

    val cv = new CrossValidator()
      .setEstimator(gbmClassifier)
      .setEvaluator(re.setLabelCol(gbmClassifier.getLabelCol)
        .setPredictionCol(gbmClassifier.getPredictionCol))
    ...
    println(cvModel.bestModel.asInstanceOf[GBMClassificationModel].getLearningRate)
    println(cvModel.bestModel.asInstanceOf[GBMClassificationModel].weights.map(_.mkString(",")).mkString(","))
